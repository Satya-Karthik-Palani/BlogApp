...
export default function Signup({isSignUp}) {
  ...
  return (
    <div className='flex justify-center mt-8 h-screen'>
        <div className='flex flex-col gap-y-2 w-1/3 bg-gray-300 rounded-lg p-4 h-[60vh] dark:bg-[#2f3335] dark:text-white'>
            {isSignUp ? (<h1 className='font-bold text-xl m-2'>Signup  : </h1>) : (<h1 className='font-bold text-xl m-2'>Log In : </h1>)}
            {isSignUp && <input onChange={(e)=>setUsername(e.target.value)} placeholder='User Name' type='text' className='m-2 p-3 border border-black rounded-lg dark:bg-[#3b3b3b]'/>}
            <input onChange={(e)=>setEmail(e.target.value)} placeholder='Email' type="email" className='m-2 p-3 border border-black rounded-lg dark:bg-[#3b3b3b]'/>
            <input onChange={(e)=>setPassword(e.target.value)} placeholder='Password' type="password" className='m-2 p-3 border border-black rounded-lg dark:bg-[#3b3b3b]'/>
